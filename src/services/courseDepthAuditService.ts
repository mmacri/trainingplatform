import type { AppData, Course } from "../data/schema";
...
const visualTypes = new Set(["learning_diagram", "process_diagram", "timeline", "network_explorer", "system_inspector", "evidence_inspector", "artifact_review", "quality_comparison"]);
const practiceTypes = new Set(["decision_cards", "classification", "sequence_builder", "matching", "quick_recall", "knowledge_check", "checklist", "record_repair"]);
const investigationTypes = new Set(["artifact_review", "investigation_activity", "system_inspector", "evidence_inspector", "record_repair"]);
...
export class CourseDepthAuditService {
  ...
  static auditCourse(data: AppData, courseId: string): CourseDepthAudit {
    ...
    const questionCount = assessments.reduce((count, assessment) => count + data.assessmentQuestions.filter((item) => item.assessmentId === assessment.id).length, 0);
    const resources = data.learningResources.filter((resource) => resource.relatedCourseIds.includes(course.id));
    const artifacts = data.trainingArtifacts.filter((artifact) => artifact.relatedCourseIds.includes(course.id));
    ...
    if (blocks.filter((block) => visualTypes.has(block.type)).length === 0) add("VISUAL", "No signature visual learning block found.", "Add a course-specific diagram, simulation, or artifact view.");
    if (blocks.filter((block) => practiceTypes.has(block.type)).length < requiredInteractions) add("PRACTICE", "The course has limited active practice.", "Add course-specific classification, sequence, inspection, or decision activities.");
    if (blocks.filter((block) => investigationTypes.has(block.type)).length === 0 && artifacts.length === 0) add("INVESTIGATION", "No realistic artifact investigation is attached.", "Add an artifact review or multi-tool investigation aligned to the course identity.");
    ...
    lessons.forEach((lesson) => {
      ...
      const active = lessonBlocks.some((block) => visualTypes.has(block.type) || practiceTypes.has(block.type) || investigationTypes.has(block.type));
    ...
    const dimensions = {
      ...
      activePractice: blocks.filter((block) => practiceTypes.has(block.type)).length >= requiredInteractions ? "STRONG" : "NEEDS_ATTENTION",
      investigation: blocks.some((block) => investigationTypes.has(block.type)) || artifacts.length ? "STRONG" : "NEEDS_ATTENTION",
